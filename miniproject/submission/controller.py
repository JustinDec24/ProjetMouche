...
class Controller:
    """Hierarchical controller for levels 0–1.

    Layer 0 — olfaction: log-space bilateral odor comparison for
              distance-independent steering toward the food source.
    Layer 1 — terrain: body orientation adjusts speed, steering, and
              adhesion on rolling hills; floating-leg detection via
              contact forces provides targeted adhesion on hilltops.
    Low level: CPG-based TurningController converts asymmetric
    descending drives into coordinated leg movements.
    """
    ...
    PALP_WEIGHT = 9
    ANTENNA_WEIGHT = 1
    STOP_LOG_THRESHOLD = -10.0
    DECISION_INTERVAL_S = 0.05

    # --- terrain awareness (Level 1) ---
    BIAS_SMOOTHING = 0.3
    ...
    def step(self, sim: MiniprojectSimulation):
        ...
        if tilt > 0.01:
            adhesion *= 1.0 + tilt * self.ADHESION_BOOST
    ...
    def _compute_drives(self, sim) -> np.ndarray:
        ...
        log_lr = np.average(
            arr, axis=0, weights=[self.PALP_WEIGHT, self.ANTENNA_WEIGHT]
        )

        if log_lr.mean() > self.STOP_LOG_THRESHOLD:
            self._stopped = True
            return np.array([0.0, 0.0])

        bias = self.ATTRACTIVE_GAIN * (log_lr[0] - log_lr[1])

        # ---- terrain correction (Level 1) ----
        pitch, roll_ind, uprightness = self._get_orientation(sim)
        downhill = max(0.0, -pitch)
        bias += -self.SLOPE_STEER_GAIN * roll_ind * downhill
